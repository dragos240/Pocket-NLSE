...
u64 filesize_to_u64(FS_Archive arch, char* filepath){
	...
	if(ret)
		error_display_res(NULL, NULL, ret, "Failed to open file.");
	...
	if(ret)
		error_display_res(NULL, NULL, ret, "Failed to close handle.");
...
int buffer_to_file(FS_Archive arch, char* buffer, u64 size, char* dirpath, char* filename){
	...
	if(ret){
		error_display_res(NULL, NULL, ret, "Failed to write file.");
		error = 1;
		goto btf_close_files;
	}
	if(written != size){
		char buf[256];
		sprintf(buf, "local %s incorrectly written to.", filename);
	...
	if(arch == game_arch){
		...
		if(ret){
			error_display_res(NULL, NULL, ret, "Failed to commit data to game archive.");
			error = 1;
		}
	}
...
char* file_to_buffer(FS_Archive arch, char* dirpath, char* filename){
	...
	if(ret){
		error_display_res(NULL, NULL, ret, "Failed to get file size.");
		error = 1;
		goto ftb_close_files;
	}
	...
	if(ret){
		error_display_res(NULL, NULL, ret, "Failed to read from file.");
		error = 1;
		goto ftb_close_files;
	}
	if(read != file_size){
		char buf[256];
		sprintf(buf, "Problem reading from %s.", filename);
		error_display(NULL, NULL, buf);
		error = 1;
		goto ftb_close_files;
	}
	...
	if(error == 1){
		error_display(NULL, NULL, "Errors occurred while running file_to_buffer.");
		free(buffer);
		return NULL;
	}
	else
...
void backup_to_prev_folder(char* dirname){
	...
	if(files.numfiles != 0){
		for(i = 0; i < files.numfiles; i++){
			...
			if(ret){
				error_display_res(NULL, NULL, ret, "Failed to delete file.");
				return;
			}
			free(filepath);
		}
	...
	for(i = 0; i < files.numfiles; i++){
		...
		if(!buf){
			error_display(NULL, NULL, "Buffer empty.");
			break;
		}
		...
		if(error == -1){
			error_display(NULL, NULL, "buffer_to_file failed.");
		}
		free(filepath);
		free(buf);
		if(error < 0)
	...
	free(sdmcpath);
	char buf2[256];
	sprintf(buf2, "Done backing up game to %s", dirname);
	error_display(NULL, NULL, buf2);
}
